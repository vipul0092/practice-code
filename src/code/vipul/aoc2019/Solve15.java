...
public class Solve15 {
    ...
    private static final long WALL_BANG = 0;
    private static final long FOUND_PLUS_MOVEMENT = 2;

    private static final Map<Long, Function<Grid.Pos, Grid.Pos>> MOVEMENT_MAP;
    private static Map<Grid.Pos, Character> GRID_POS;
    private static Grid.Pos oxygenPos;
    private static int maxi = 0, maxj = 0, mini = 0, minj = 0;
    private static int maxRows = -1, maxCols = -1;
    private static char[][] grid;
    private static int maxSteps = 0;

    static {
        MOVEMENT_MAP = new HashMap<>();
        MOVEMENT_MAP.put(NORTH, pos -> pos.moveUp());
        MOVEMENT_MAP.put(SOUTH, pos -> pos.moveDown());
        MOVEMENT_MAP.put(WEST, pos -> pos.moveLeft());
        MOVEMENT_MAP.put(EAST, pos -> pos.moveRight());
    }

    public static void solve() {
        visited = new HashSet<>();
        connections = new HashMap<>();
        GRID_POS = new HashMap<>();
        Computer computer = Computer.make();
        computer.loadProgramInMemory(input);

        Grid.Pos currentPosition = Grid.Pos.of(0, 0);
        GRID_POS.put(currentPosition, ' ');

        walk(computer, currentPosition, null, 0);

        System.out.println("Answer: " + answer); // 234
    }
    ...
    public static void solvePart2() {
        ...
        int i = 0, j = 0, oxygenI = -1, oxygenJ = -1;
        for (int a = mini; a <= maxi; a++, i++) {
            j = 0;
            for (int b = minj; b <= maxj; b++, j++) {
                Grid.Pos pos = Grid.Pos.of(a, b);
                grid[i][j] = GRID_POS.getOrDefault(pos, ' ');
                if (pos.equals(oxygenPos)) {
                    oxygenI = i;
                    oxygenJ = j;
                }
            }
        ...
        List<Grid.Pos> start = new ArrayList<>();
        start.add(Grid.Pos.of(oxygenI, oxygenJ));
        spread(start, 0);
        System.out.println("Answer: " + maxSteps); // 292
    }

    private static void spread(List<Grid.Pos> positions, int steps) {
        maxSteps = Math.max(maxSteps, steps);
        List<Grid.Pos> spreadInto = new ArrayList<>();
        for (Grid.Pos position: positions) {
            List<Grid.Pos> neighbours = getNeighbours(position);
            for (Grid.Pos neighbour: neighbours) {
                if (grid[neighbour.i()][neighbour.j()] == ' ') {
                    grid[neighbour.i()][neighbour.j()] = 'O';
                    spreadInto.add(neighbour);
                }
            }
        }
        if (spreadInto.size() > 0) {
            showGrid();
            spread(spreadInto, steps + 1);
        }
    }

    private static List<Grid.Pos> getNeighbours(Grid.Pos pos) {
        List<Grid.Pos> neighbours = new ArrayList<>();
    ...
    private static void walk(Computer computer, Grid.Pos currentPos, Grid.Pos prevPos, int lengthUntilNow) {

        if (visited.contains(currentPos)) {
            if (prevPos != null) {
                // Go back to prev position
                Long direction = connections.get(currentPos).get(prevPos);
                computer.giveInputAndExecute(direction);
        ...
        maxi = Math.max(currentPos.i(), maxi);
        mini = Math.min(currentPos.i(), mini);
        maxj = Math.max(currentPos.j(), maxj);
        minj = Math.min(currentPos.j(), minj);

        // Try walking in different directions if its allowed
        for(Map.Entry<Long, Function<Grid.Pos, Grid.Pos>> entry : MOVEMENT_MAP.entrySet()) {
            Long movementCode = entry.getKey();
            Function<Grid.Pos, Grid.Pos> positionChanger = entry.getValue();
            computer.getInput().enqueueInput(movementCode);
            computer.execute();

            long output = computer.getOutput().getOutputs().get(0);
            computer.getOutput().clear();
            Grid.Pos newPosition = positionChanger.apply(currentPos);

            GRID_POS.put(newPosition, '#');
            if (output != WALL_BANG) {
                connections.computeIfAbsent(currentPos, (i) -> new HashMap<>());
                connections.get(currentPos).put(newPosition, movementCode);

                GRID_POS.put(newPosition, ' ');
                if (output == FOUND_PLUS_MOVEMENT) {
                    answer = Math.min(lengthUntilNow + 1, answer);
                    GRID_POS.put(newPosition, 'O');
                    oxygenPos = newPosition;
                }
                walk(computer, newPosition, currentPos, lengthUntilNow + 1);
            }
        }
