...
public class Solve4 {
    ...
    public static void solve() {
        Map<String, String> timeToEventMap = new TreeMap<>();
        Map<Integer, Integer> guardSleepTimes = new HashMap<>();
        Map<Integer, BIT_1D_PointQuery> guardPerMinuteSleepTimes = new HashMap<>();
        String[] events = input.split("\n");

        for (String event : events) {
            int indexOfClosingBracket = event.lastIndexOf(']');
            timeToEventMap.put(event.substring(1, indexOfClosingBracket),
                    event.substring(indexOfClosingBracket + 1).trim());
        }

        Function<String, Integer> getMinute = (s) -> Integer.parseInt(s.split(" ")[1].trim().split(":")[1]);
        int currentGuardNumber = -1;
        int lastSleepMinute = -1;

        int mostSleptGuard = -1;
        int mostSleptMinutes = -1;

        for (var entry : timeToEventMap.entrySet()) {
            String timestamp = entry.getKey();
            String event = entry.getValue();

            if (event.startsWith("Guard")) {
                currentGuardNumber = Integer.parseInt(event.split(" ")[1].trim().substring(1));
            } else if (event.startsWith("falls")) {
                lastSleepMinute = getMinute.apply(timestamp);
            } else if (event.startsWith("wakes")) {
                int currentMinutes = getMinute.apply(timestamp);

                guardSleepTimes.putIfAbsent(currentGuardNumber, 0);
                int totalSleptMinutes = guardSleepTimes.get(currentGuardNumber) + currentMinutes - lastSleepMinute;
                guardSleepTimes.put(currentGuardNumber, totalSleptMinutes);

                if (totalSleptMinutes > mostSleptMinutes) {
                    mostSleptMinutes = totalSleptMinutes;
                    mostSleptGuard = currentGuardNumber;
                }

                guardPerMinuteSleepTimes.putIfAbsent(currentGuardNumber, BIT_1D_PointQuery.ofSize(60));
                guardPerMinuteSleepTimes.get(currentGuardNumber).range_add(lastSleepMinute + 1, currentMinutes, 1);
            }
        }

        int answer = -1;
        int maxTimes = -1;
        BIT_1D_PointQuery queryMostSleptGuard = guardPerMinuteSleepTimes.get(mostSleptGuard);
        for (int i = 1; i < 60; i++) {
            int minutesSlept = queryMostSleptGuard.point_query(i);
            if (minutesSlept > maxTimes) {
                maxTimes = minutesSlept;
                answer = i - 1;
            }
        }
        System.out.println("Answer: " + (answer * mostSleptGuard)); // 65489
    }
    ...
    public static void solvePart2() {
        ...
        int maxTimes = -1;
        int selectedGuard = -1, selectedMinute = -1;


        for (var entry : guardPerMinuteSleepTimes.entrySet()) {
            int guard = entry.getKey();
            BIT_1D_PointQuery minuteQuery = entry.getValue();
            for (int i = 1; i < 60; i++) {
                int times = minuteQuery.point_query(i);
                if (times > maxTimes) {
                    maxTimes = times;
                    selectedGuard = guard;
                    selectedMinute = i - 1;
        ...
        System.out.println("Answer: " + (selectedGuard * selectedMinute)); // 3852
    }
}
